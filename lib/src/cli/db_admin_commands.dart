...
class DBTableExportCommand extends FlintCommand {
  ...
  Future<void> execute(List<String> args) async {
    if (args.isEmpty) {
      Log.debug(
          '❌ Missing table name. Use --db-table-export <table> [output.sql]');
      exit(1);
    }

    final table = args.first.trim();
    if (!_isSafeIdentifier(table)) {
      Log.debug(
          '❌ Invalid table name "$table". Use letters, numbers, and underscores only.');
      exit(1);
    }
    ...
    final stamp =
        '${now.year}${_two(now.month)}${_two(now.day)}_${_two(now.hour)}${_two(now.minute)}${_two(now.second)}';
    final outputFile =
        args.length > 1 ? args[1] : 'db_export_${table}_$stamp.sql';
